...
## 6. 累计对外担保余额（2026-09-26）
担保公告中的“对合并报表外单位担保余额”生成 `guarantee_balance` 信号（严重程度 info，有效 12 个月），
只用于承压评分的“对外担保”维度：敞口 = max(最新披露的累计对外余额, 有效期内非子公司担保合计)，
除以最新一期净资产。不使用“担保总额”，因为它包含对自己子公司的担保。
